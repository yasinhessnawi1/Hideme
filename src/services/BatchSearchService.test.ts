    ...
    describe('batchSearch', () => {
        test('should execute a batch search with default options', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith({
                method: 'POST',
                url: expect.stringContaining('/batch/search'),
                formData: expect.any(FormData)
            });

            // Verify formData content
            const formDataArg = (apiRequest as any).mock.calls[0][0].formData;
            expect(formDataArg.get('search_terms')).toBe(searchTerm);
            expect(formDataArg.get('case_sensitive')).toBe('false');
        ...
        test('should include case_sensitive and regex options when specified', async () => {
            // Mock response (simplified for this test)
            const mockResponse: BatchSearchResponse = {
                batch_summary: {
                    batch_id: 'test-batch-123',
                    total_files: 1,
            ...
            // Verify API called with correct options
            const formDataArg = (apiRequest as any).mock.calls[0][0].formData;
            expect(formDataArg.get('case_sensitive')).toBe('true');
            expect(formDataArg.get('ai_search')).toBe('true');
        });

        /*
        test('should accept alternative option names (isCaseSensitive and isAiSearch)', async () => {
            // Mock response (simplified for this test)
            const mockResponse: BatchSearchResponse = {
                batch_summary: {
                    batch_id: 'test-batch-123',
            ...
            // Options with alternative property names
            const options = {
            ...
            const formDataArg = (apiRequest as any).mock.calls[0][0].formData;
            expect(formDataArg.get('case_sensitive')).toBe('true');
            expect(formDataArg.get('regex')).toBe('true');
        });
        */
        ...
        test('should handle API error', async () => {
            ...
            (apiRequest as any).mockRejectedValue(new Error(errorMessage));
