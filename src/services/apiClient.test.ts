...
// Mock dependencies
vi.mock('../services/apiCacheService', () => ({
    default: {
        get: vi.fn(),
        post: vi.fn(),
        put: vi.fn(),
...
describe('apiClient', () => {
    ...
    describe('get', () => {
        ...
        */
        ...
        test('should use different TTL values based on URL', async () => {
            // Setup mock
            (apiCache.get as any).mockResolvedValueOnce({ data: 'auth data' });
            (apiCache.get as any).mockResolvedValueOnce({ data: 'user data' });
            (apiCache.get as any).mockResolvedValueOnce({ data: 'settings data' });
            (apiCache.get as any).mockResolvedValueOnce({ data: 'entities data' });
            (apiCache.get as any).mockResolvedValueOnce({ data: 'other data' });
            ...
            const authTtl = (apiCache.get as any).mock.calls[0][2].ttl;
            const userTtl = (apiCache.get as any).mock.calls[1][2].ttl;
            const settingsTtl = (apiCache.get as any).mock.calls[2][2].ttl;
            const entitiesTtl = (apiCache.get as any).mock.calls[3][2].ttl;
            const defaultTtl = (apiCache.get as any).mock.calls[4][2].ttl;
    ...
    describe('post', () => {
        test('should call apiCache.post with correct parameters', async () => {
            // Setup mock
            (apiCache.post as any).mockResolvedValueOnce({ data: 'created' });
            (authService.getToken as any).mockReturnValueOnce('test-token');
        ...
        test('should support cacheable option for POST requests', async () => {
            ...
            const ttl = (apiCache.post as any).mock.calls[0][2].ttl;
