...
import {EncryptedApiResponse} from "../../types";
...
export class BatchSearchService {
    ...
    static async batchSearch(
        ...
        try {
            ...
            // Encrypt required fields: files, search_terms
            const fieldsToEncrypt = ['files', 'search_terms'];
            ...
            // Execute the API call with encrypted data
            const apiResponse = await apiRequest<BatchSearchResponse | EncryptedApiResponse>({
                method: 'POST',
            ...
            // Check if response is encrypted by checking for encrypted_data property
            let result: BatchSearchResponse;
            if ('encrypted_data' in apiResponse) {
                // It's an encrypted response, decrypt it
                result = await batchEncryptionService.decrypt(apiResponse.encrypted_data) as BatchSearchResponse;
            } else {
                // It's already a BatchSearchResponse
                result = apiResponse as BatchSearchResponse;
            }
            ...
            try {
                ...
                console.warn('[BatchSearchService] Error during API key cleanup:', cleanupError);
            }
        }
    }
