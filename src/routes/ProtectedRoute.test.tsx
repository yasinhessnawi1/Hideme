...
describe('ProtectedRoute Component', () => {
    ...
    it('redirects to login page when user is not authenticated', async () => {
        ...
        // Wait for verification to complete and navigate to be called
        await waitFor(() => {
            expect(mockVerifySession).toHaveBeenCalled();
    ...
    it('does not verify session if no token exists in localStorage', async () => {
        ...
        // The verification might still be called, but it should quickly determine there's no token
        await waitFor(() => {
            expect(mockNavigate).toHaveBeenCalledWith('/login', {
                state: { from: '/current-path' }
            });
    ...
    it('handles errors during verification process', async () => {
        ...
        // Wait for verification to fail and navigate to be called
        await waitFor(() => {
            expect(mockVerifySession).toHaveBeenCalled();
            expect(mockNavigate).toHaveBeenCalledWith('/login', {
                state: { from: '/current-path' }
    ...
    it('uses custom redirect path when provided', async () => {
        ...
        // Wait for navigate to be called with custom path
        await waitFor(() => {
    ...
    it('preserves the original path in redirect state for post-login navigation', async () => {
        mockVerifySession.mockResolvedValue(false);
        ...
        // Check that redirect state includes the 'from' path
        await waitFor(() => {
            expect(mockNavigate).toHaveBeenCalledWith('/login', {
                state: { from: '/current-path' }
            });
