...
describe('SummaryPersistenceStore', () => {
    ...
    describe('Analyzed Files Management', () => {
        ...
        test('addAnalyzedFile should not duplicate existing files', () => {
            // Setup initial state
            localStorageMock.setItem('entity-detection-analyzed-files', JSON.stringify(['existingFile.pdf']));

            // Add the same file again
            summaryPersistenceStore.addAnalyzedFile('entity', 'existingFile.pdf');

            // Verify the set still has just one entry
            const entityFiles = summaryPersistenceStore.getAnalyzedFiles('entity');
            expect(entityFiles.size).toBe(1);

            // Verify localStorage was still updated (called for both getting and setting)
            expect(localStorageMock.setItem).toHaveBeenCalledTimes(1);
        });

        test('removeAnalyzedFile should remove a file from the analyzed set', () => {
            // Setup initial state
            localStorageMock.setItem('entity-detection-analyzed-files',
                JSON.stringify(['file1.pdf', 'file2.pdf', 'file3.pdf'])
            );

            // Remove a file
            summaryPersistenceStore.removeAnalyzedFile('entity', 'file2.pdf');

            // Verify file was removed
            const entityFiles = summaryPersistenceStore.getAnalyzedFiles('entity');
            expect(entityFiles.size).toBe(2);
            expect(entityFiles.has('file1.pdf')).toBe(true);
            expect(entityFiles.has('file2.pdf')).toBe(false);
            expect(entityFiles.has('file3.pdf')).toBe(true);
            ...
            expect(localStorageMock.setItem).toHaveBeenCalledWith(
                'entity-detection-analyzed-files',
                expect.not.stringContaining('file2.pdf')
            );
        });

        test('removeAnalyzedFile should handle non-existent files gracefully', () => {
            // Setup initial state
            localStorageMock.setItem('entity-detection-analyzed-files', JSON.stringify(['file1.pdf']));

            // Try to remove a non-existent file
            summaryPersistenceStore.removeAnalyzedFile('entity', 'nonexistent.pdf');
            ...
            // Verify localStorage was still updated (called during get and set)
            expect(localStorageMock.setItem).toHaveBeenCalledTimes(1);
        });

        test('reconcileAnalyzedFiles should keep only existing files', () => {
            // Setup initial state with files that may or may not exist
            localStorageMock.setItem('entity-detection-analyzed-files',
                JSON.stringify(['file1.pdf', 'removed.pdf', 'file3.pdf'])
            );

            // Define which files actually exist
            const availableFileKeys = ['file1.pdf', 'file2.pdf', 'file3.pdf']; // removed.pdf is missing

            // Reconcile files
            summaryPersistenceStore.reconcileAnalyzedFiles('entity', availableFileKeys);

            // Verify only existing files remain
            const entityFiles = summaryPersistenceStore.getAnalyzedFiles('entity');
            expect(entityFiles.size).toBe(2);
            expect(entityFiles.has('file1.pdf')).toBe(true);
            expect(entityFiles.has('removed.pdf')).toBe(false);
            expect(entityFiles.has('file3.pdf')).toBe(true);
            ...
            expect(localStorageMock.setItem).toHaveBeenCalledWith(
                'entity-detection-analyzed-files',
                expect.not.stringContaining('removed.pdf')
            );
        });

        test('reconcileAnalyzedFiles should return original set if no files provided', () => {
            // Setup initial state
            localStorageMock.setItem('entity-detection-analyzed-files',
                JSON.stringify(['file1.pdf', 'file2.pdf'])
            );

            // Reconcile with empty array
            const result = summaryPersistenceStore.reconcileAnalyzedFiles('entity', []);

            // Verify original set returned unchanged
            expect(result.size).toBe(2);
            expect(result.has('file1.pdf')).toBe(true);
            expect(result.has('file2.pdf')).toBe(true);

            // Verify localStorage was not updated
            expect(localStorageMock.setItem).not.toHaveBeenCalledWith('entity-detection-analyzed-files', expect.anything());
        });
    });
    ...
    describe('File Summaries Management', () => {
        ...
        test('removeFileFromSummaries should remove file from summaries and analyzed files', () => {
            // Setup initial state
            const summaries = [
                {
                    fileKey: 'file1.pdf',
                ...
                },
                {
                    fileKey: 'file2.pdf',
                    fileName: 'File 2',
                    entities_detected: { total: 8 },
                    performance: { sanitize_time: 400 }
                }
            ];
            ...
            // Remove file1.pdf
            summaryPersistenceStore.removeFileFromSummaries('entity', 'file1.pdf');
            ...
            // Verify analyzed files also updated
            const entityFiles = summaryPersistenceStore.getAnalyzedFiles('entity');
            expect(entityFiles.size).toBe(1);
            expect(entityFiles.has('file1.pdf')).toBe(false);
            expect(entityFiles.has('file2.pdf')).toBe(true);
            ...
            expect(window.dispatchEvent).toHaveBeenCalledWith(
                expect.objectContaining({
                    type: 'entity-file-removed',
                    detail: expect.objectContaining({ fileKey: 'file1.pdf' })
                })
            );
        });

        test('reconcileFileSummaries should keep only summaries for existing files', () => {
            // Setup initial state
            const summaries = [
                {
                    fileKey: 'file1.pdf',
            ...
            localStorageMock.setItem('search-file-summaries', JSON.stringify(summaries));

            // Define files that actually exist
            const availableFiles = [
                new File(['content'], 'file1.pdf', { type: 'application/pdf' })
                // removed.pdf is missing
            ];

            // Reconcile summaries
            summaryPersistenceStore.reconcileFileSummaries('search', availableFiles);
        ...
        test('reconcileFileSummaries should return original array if no files provided', () => {
            // Setup initial state
            const summaries = [
                {
                    fileKey: 'file1.pdf',
            ...
            // Reconcile with empty array
            const result = summaryPersistenceStore.reconcileFileSummaries('search', []);

            // Verify original array returned unchanged
            expect(result).toHaveLength(1);
            expect(result[0].fileKey).toBe('file1.pdf');

            // Verify localStorage was not updated
            expect(localStorageMock.setItem).not.toHaveBeenCalledWith('search-file-summaries', expect.anything());
        });
    ...
    describe('Search Queries Management', () => {
        ...
        test('removeSearchQuery should remove queries by fileKey if specified', () => {
            const queries = [
                { query: 'query1', fileKey: 'file1.pdf' },
                { query: 'query2', fileKey: 'file1.pdf' },
                { query: 'query3', fileKey: 'file2.pdf' }
            ];

            localStorageMock.setItem('search-active-queries', JSON.stringify(queries));

            // Remove query for specific file
            summaryPersistenceStore.removeSearchQuery({ query: 'query1' }, 'file1.pdf');

            // Verify only the specific query for that file was removed
            const updatedQueries = JSON.parse(localStorageMock.getItem('search-active-queries') || '[]');
            expect(updatedQueries).toHaveLength(2);
            expect(updatedQueries[0].query).toBe('query2');
            expect(updatedQueries[1].query).toBe('query3');
        });

        test('clearSearchQueries should clear all search related data for a file', () => {
            // Setup mock data
            const searchQueries = [
                { query: 'query1', fileKey: 'file1.pdf' },
                { query: 'query2', fileKey: 'file1.pdf' },
            ...
            localStorageMock.setItem('search-active-queries', JSON.stringify(searchQueries));
            localStorageMock.setItem('search-analyzed-files', JSON.stringify(['file1.pdf', 'file2.pdf']));
            localStorageMock.setItem('search-file-summaries', JSON.stringify([
                { fileKey: 'file1.pdf', fileName: 'File 1', matchCount: 5 },
                { fileKey: 'file2.pdf', fileName: 'File 2', matchCount: 3 }
            ]));

            // Mock clearSearch function
            const clearSearch = vi.fn();

            // Clear search queries for file1.pdf
            summaryPersistenceStore.clearSearchQueries('file1.pdf', clearSearch);

            // Verify file was removed from analyzed files
            expect(localStorageMock.setItem).toHaveBeenCalledWith(
                'search-analyzed-files',
                expect.not.stringContaining('file1.pdf')
            );

            // Verify file summary was removed
            expect(localStorageMock.setItem).toHaveBeenCalledWith(
                'search-file-summaries',
                expect.not.stringContaining('file1.pdf')
            );

            // Verify clearSearch was called for each query for that file
